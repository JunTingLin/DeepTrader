...
class PortfolioSim(object):
    ...
    def _step(self, w0, ror, p):
        ...
        if self.allow_short:
            ...
            # Modified: short cash is kept as reserve, not reinvested into long positions
            dv0_long = (dv0 * p) # (batch, )
            # Adjust previous long weights based on new allocation
            # When p=0, ratio would be 0/0, so use 1.0 as default (no positions anyway)
            adjustment_ratio = np.where(dv0_long > EPS, (dv0 * p) / dv0_long, 1.0)
            dw0_long = dw0 * adjustment_ratio[..., None] # adjest long position weights # (batch, num_assets)
            ...
            # Modified: add short cash reserve to final value
            dv1 = LongPosition_value - (ShortPosition_value) / (1 - self.fee) \
                  + dv0_short + dv0_short_after_sale
            # short sell p&l = dv0_short - returned value
            # idle cash from short selling = dv0_short_after_sale

            rate_of_return = dv1 / dv0 - 1
            cash_value = dv0_short_after_sale
            stocks_value = dv1 - dv0_short_after_sale
