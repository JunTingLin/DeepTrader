...
def plot_stock_price_trends(experiment_id, outputs_base_path, stock_symbols, sample_dates, period='test', save_plots=True):
    """
    ...
    output_dir = f'stock_price_plots/{experiment_id}/{period}'
    ...
    for stock_idx, stock_symbol in enumerate(stock_symbols):
        ...
        for step_idx in range(n_steps + 1):  # n_steps + 1 lines for n_steps intervals
            ...
            if step_date_idx < len(period_dates):
                ...
                plt.axvline(x=step_date, color='gray', linestyle='--', alpha=0.5, linewidth=0.8)
...
def main():
    ...
    for exp_id in EXPERIMENT_IDS:
        ...
        # Plot stock price trends with trading positions
        ...
        plot_stock_price_trends(exp_id, OUTPUTS_BASE_PATH, symbols, df_val.index, 'val', save_plots=True)
        plot_stock_price_trends(exp_id, OUTPUTS_BASE_PATH, symbols, df_test.index, 'test', save_plots=True)
