...
def main():
    ...
    for exp_id in EXPERIMENT_IDS:
        ...
        if val_msu and val_msu.get('total_predictions', 0) > 0:
            print(f"  Validation Period:")
            print(f"    Overall Accuracy: {val_msu['overall_accuracy']:.2%} ({val_msu['correct_predictions']}/{val_msu['total_predictions']} correct)")
            print("")

            # Bullish metrics
            if val_msu['predicted_bullish'] > 0:
                print(f"    Bullish Precision: {val_msu['bullish_precision']:.2%} ({val_msu['bullish_tp']}/{val_msu['predicted_bullish']} when predicted bullish, market went up)")
            if val_msu['actual_up'] > 0:
                print(f"    Bullish Recall:    {val_msu['bullish_recall']:.2%} ({val_msu['bullish_tp']}/{val_msu['actual_up']} when market went up, predicted bullish)")

            # Bearish metrics
            if val_msu['predicted_bearish'] > 0:
                print(f"    Bearish Precision: {val_msu['bearish_precision']:.2%} ({val_msu['bearish_tp']}/{val_msu['predicted_bearish']} when predicted bearish, market went down)")
            if val_msu['actual_down'] > 0:
                print(f"    Bearish Recall:    {val_msu['bearish_recall']:.2%} ({val_msu['bearish_tp']}/{val_msu['actual_down']} when market went down, predicted bearish)")

            # Neutral metrics (if any)
            if val_msu['predicted_neutral'] > 0:
                print(f"    Neutral Precision: {val_msu['neutral_precision']:.2%} ({val_msu['neutral_tp']}/{val_msu['predicted_neutral']} when predicted neutral, market flat)")
            if val_msu['actual_flat'] > 0:
                print(f"    Neutral Recall:    {val_msu['neutral_recall']:.2%} ({val_msu['neutral_tp']}/{val_msu['actual_flat']} when market flat, predicted neutral)")

        # Testing period
        test_msu = calculate_msu_market_accuracy(f"{OUTPUTS_BASE_PATH}/{exp_id}", 'test')
        if test_msu and test_msu.get('total_predictions', 0) > 0:
            print(f"  Testing Period:")
            print(f"    Overall Accuracy: {test_msu['overall_accuracy']:.2%} ({test_msu['correct_predictions']}/{test_msu['total_predictions']} correct)")
            print("")

            # Bullish metrics
            if test_msu['predicted_bullish'] > 0:
                print(f"    Bullish Precision: {test_msu['bullish_precision']:.2%} ({test_msu['bullish_tp']}/{test_msu['predicted_bullish']} when predicted bullish, market went up)")
            if test_msu['actual_up'] > 0:
                print(f"    Bullish Recall:    {test_msu['bullish_recall']:.2%} ({test_msu['bullish_tp']}/{test_msu['actual_up']} when market went up, predicted bullish)")

            # Bearish metrics
            if test_msu['predicted_bearish'] > 0:
                print(f"    Bearish Precision: {test_msu['bearish_precision']:.2%} ({test_msu['bearish_tp']}/{test_msu['predicted_bearish']} when predicted bearish, market went down)")
            if test_msu['actual_down'] > 0:
                print(f"    Bearish Recall:    {test_msu['bearish_recall']:.2%} ({test_msu['bearish_tp']}/{test_msu['actual_down']} when market went down, predicted bearish)")

            # Neutral metrics (if any)
            if test_msu['predicted_neutral'] > 0:
                print(f"    Neutral Precision: {test_msu['neutral_precision']:.2%} ({test_msu['neutral_tp']}/{test_msu['predicted_neutral']} when predicted neutral, market flat)")
            if test_msu['actual_flat'] > 0:
                print(f"    Neutral Recall:    {test_msu['neutral_recall']:.2%} ({test_msu['neutral_tp']}/{test_msu['actual_flat']} when market flat, predicted neutral)")
