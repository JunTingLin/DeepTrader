...
使用 TCN + TE_2D (Transformer) + Spatial Attention 的監督學習預訓練
"""
...
class ASU_TE2D_Wrapper(nn.Module):
    """
    簡單包裝 ASU（TE2D 版本），支持可選的 regression head
    """
...
def main(args):
    ...
    run_name = f"ASU_TE2D_{args.loss_function}_w{args.window_len}_h{args.horizon}_{timestamp}"
    ...
    # Create ASU model (TE2D version: transformer_asu_bool=True)
    asu_base = ASU(
        ...
        aptinit=None,
        transformer_asu_bool=True,  # TE2D 版本
        num_assets=num_stocks
    ).to(device)

    # Wrap with regression head if needed
    model = ASU_TE2D_Wrapper(asu_base, add_regression_head=need_predictions).to(device)
